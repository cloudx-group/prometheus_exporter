...
from src.srclibs.common import MetricCollector, CommonMetricCollector
from src.srclibs.scraper import ScrapeMetrics
...
class ResourceManagerMetricCollector(MetricCollector):
    ...
    def get_metrics(self, beans):
        try:
            for i in range(len(beans)):
                ...
                if 'QueueMetrics' in beans[i]['name'] and self.get_tag_in_queue(beans[i]):
    ...
    def get_tag_in_queue(self, d):
        tag_queue_value = d.get('tag.Queue')
        if tag_queue_value:
            return re.match(self.queue_regexp, tag_queue_value)
        else:
            return False
